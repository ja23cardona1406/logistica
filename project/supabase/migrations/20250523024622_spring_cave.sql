/*
  # Create Repremundo Database Schema

  1. New Tables
    - `profiles` - User profiles with roles
    - `sessions` - Work sessions for operators
    - `shipments` - Shipment data with tracking codes
    - `shipment_items` - Items within shipments
    - `processes` - Process templates for import/export
    - `process_steps` - Steps within each process
    - `process_executions` - Instances of processes being executed
    - `process_step_executions` - Execution of individual steps
    - `alerts` - System alerts for administrators
    - `exemplary_processes` - Gallery of well-done processes
    - `assistant_logs` - Logs of virtual assistant interactions
  
  2. Security
    - Enable RLS on all tables
    - Add policies for authentication and authorization
    - Separate policies for operators and administrators
...
INSERT INTO process_steps (process_id, order, title, description, is_critical)
VALUES
